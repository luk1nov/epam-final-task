...
public class ValidatorImpl implements Validator {
    ...
    private static final String LOCALE_PATTERN = "^en$|^ru$";
    private static final String DATE_RANGE_PATTERN = "^\\d{4}-\\d{2}-\\d{2}:\\d{4}-\\d{2}-\\d{2}$|^\\d{4}-\\d{2}-\\d{2}$";
    ...
    @Override
    public boolean isValidSearchPattern(String searchQuery) {
        return searchQuery != null && searchQuery.matches(SEARCH_PATTERN);
    }
