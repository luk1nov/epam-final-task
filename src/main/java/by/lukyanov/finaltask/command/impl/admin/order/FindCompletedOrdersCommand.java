...
public class FindCompletedOrdersCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        try {
            int pagesCount = ResultCounter.countPages(orderService.countOrdersByStatus(OrderStatus.FINISHED));
            if(currentResultPage == null || currentResultPage.isBlank()){
                currentResultPage = "1";
            }
            request.setAttribute(PAGES_COUNT, pagesCount);
            request.setAttribute(RESULT_PAGE, currentResultPage);
            List<Order> orders = orderService.findOrdersByOrderStatus(OrderStatus.FINISHED, currentResultPage);
